...
#define AlwaysOverwriteFileQuery L"SELECT `File` FROM `PSW_AlwaysOverwriteFile`"
enum eAlwaysOverwriteFileQuery { File = 1 };
...
extern "C" UINT __stdcall AlwaysOverwriteFile(MSIHANDLE hInstall)
{
	...
	BreakExitOnFailure(hr, "Table does not exist 'PSW_MsiSqlQuery'. Have you authored 'PanelSw:AlwaysOverwriteFile' entries in WiX code?");

	// Execute view
	hr = WcaOpenExecuteView(AlwaysOverwriteFileQuery, &hView);
	...
	while ((hr = WcaFetchRecord(hView, &hRecord)) != E_NOMOREITEMS)
	{
		...
		hr = WcaGetRecordString(hRecord, eAlwaysOverwriteFileQuery::File, (LPWSTR*)file);
