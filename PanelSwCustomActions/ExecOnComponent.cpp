...
HRESULT CExecOnComponent::LogProcessOutput(HANDLE hProcess, HANDLE hStdErrOut, LPCWSTR szObfuscateLog, LPWSTR* pszText)
{
...
HRESULT CExecOnComponent::Impersonate(BOOL bImpersonate, LPCWSTR szDomain, LPCWSTR szUser, LPCWSTR szPassword, CWixString* pszEnvironmentMultiSz, IMPERSONATION_CONTEXT *pctxImpersonate)
{
	...
	if (szUser && *szUser)
	{
		...
		hr = PathCreateTempFile(nullptr, L"ExecOn%u.exe", INFINITE, FILE_ATTRIBUTE_NORMAL, &pctxImpersonate->szExeWrapper, &hTempFile);
