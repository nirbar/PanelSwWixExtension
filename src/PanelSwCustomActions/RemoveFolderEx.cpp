...
static void HandleLongPaths(LPWSTR* pszLongPaths, UINT cLongPaths, RemoveFolderExLongPathHandling longPathHandling)
{
	...
	{
		CErrorPrompter warnPrompt(PSW_MSI_MESSAGES::PSW_MSI_MESSAGES_LONG_PATHS_WARNING, (INSTALLMESSAGE)(INSTALLMESSAGE::INSTALLMESSAGE_WARNING | MB_YESNOCANCEL | MB_DEFBUTTON1 | MB_ICONWARNING), S_YES, PSW_ERROR_HANDLING::PSW_ERROR_HANDLING_PROMPT);
	...
	for (UINT i = 0; i < cLongPaths; ++i)
	{
		...
		hr = PathCreateTempFile(szParentPath, L"RFE%05i.tmp", INFINITE, L"RFE", FILE_ATTRIBUTE_NORMAL, (LPWSTR*)szDst, nullptr);
