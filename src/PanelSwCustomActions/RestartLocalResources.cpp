...
HRESULT CRestartLocalResources::RegisterWithRm(const std::list<LPWSTR>& lstFolders)
{
	...
	std::map<DWORD, LPWSTR> mapServiceId;
	...
	{
		WcaLog(LOGLEVEL::LOGMSG_STANDARD, "Registering service '%ls' (%u) with RestartManager", svcId.second, svcId.first);

		hr = RmuAddService(pSession, svcId.second);
		ExitOnFailure(hr, "Failed adding service %u '%ls' to RestartManager", svcId.first, svcId.second);
	}
...
LExit:
	...
	for (std::pair<DWORD, LPWSTR> f : mapServiceId)
	{
		ReleaseStr(f.second);
	}
...
HRESULT CRestartLocalResources::EnumerateLocalProcesses(const std::list<LPWSTR>& lstFolders, std::map<DWORD, LPWSTR>& mapProcId, std::map<DWORD, LPWSTR>& mapServiceId)
{
	HRESULT hr = S_OK;
	...
	std::map<DWORD, LPWSTR> mapAllServices;

	hr = Initialize();
	ExitOnFailure(hr, "Failed to initialize");

	hr = GetServices(mapAllServices);
	ExitOnFailure(hr, "Failed to get services");
	...
	for (BOOL fContinue = ::Process32FirstW(hSnap, &peData); fContinue; fContinue = ::Process32NextW(hSnap, &peData))
	{
		...
		if (std::any_of(lstFolders.begin(), lstFolders.end(), visInFolder_))
		{
			...
			if (it != mapAllServices.end())
			{
				...
				mapServiceId[peData.th32ProcessID] = szProcessName;
...
HRESULT CRestartLocalResources::GetServices(std::map<DWORD, LPWSTR>& mapServiceId)
{
	HRESULT hr = S_OK;
	BOOL bRes = TRUE;
	...
	{
		hr = StrAllocString(&szServiceName, pServices[i].lpServiceName, 0);
		ExitOnFailure(hr, "Failed to allocate string");

		mapServiceId[pServices[i].ServiceStatusProcess.dwProcessId] = szServiceName;
